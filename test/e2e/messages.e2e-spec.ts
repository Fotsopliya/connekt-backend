...
describe('Messages e2e', () => {
  ...
  it('send, inbox/sent, conversation, markRead', async () => {
    ...
    const vehResp = await request(app.getHttpServer())
      ...
      .send({ brand: 'Peugeot', model: '208', year: 2020, color: 'blue', plateNumber: 'MSG-001' })
      .expect(201);
    ...
    const sendResp = await request(app.getHttpServer())
      .post('/messages')
      .set('x-extl-id', passengerExtl)
      .send({ tripId, recipientId: tripResp.body.driver?.id ?? undefined, content: 'Bonjour!' })
      .expect(201);
    const msgId = sendResp.body.id as string;
    ...
    await request(app.getHttpServer())
      ...
      .set('x-extl-id', driverExtl)
      .query({ with: inbox.body[0]?.sender?.id, tripId })
      .expect(200);
