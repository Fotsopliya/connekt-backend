...
function onceEvent<T = any>(socket: Socket, event: string, timeoutMs = 5000): Promise<T> {
  return new Promise((resolve, reject) => {
...
describe('Notifications (e2e, sockets)', () => {
  ...
  it('message:new delivered to recipient; booking:new/status; ticket events; trip:validated; kyc:status + verified', async () => {
    ...
    for (const extlId of [driverExtl, passengerExtl, otherExtl]) {
      ...
      await request(app.getHttpServer()).post('/auth/webhooks/clerk').set(headers).send(payload).expect(200);
    }
    ...
    const vehResp = await request(app.getHttpServer())
      ...
      .send({ brand: 'Peugeot', model: '208', year: 2021, color: 'blue', plateNumber: 'SOCK-001' })
      .expect(201);
    const vehicleId = vehResp.body.id as string;

    await request(app.getHttpServer()).post('/users/kyc').set('x-extl-id', driverExtl).send({ notes: 'verify me' }).expect(201);
    await request(app.getHttpServer()).post(`/vehicles/${vehicleId}/kyc`).set('x-extl-id', driverExtl).expect(201);
    ...
    const kycStatusP = onceEvent(passengerSock, 'kyc:status').catch(() => null); // passenger should not receive
    const userVerifiedP = onceEvent(driverSock, 'user:verified');
    ...
    const bookingStatusToPassengerP = onceEvent(passengerSock, 'booking:status');
    await request(app.getHttpServer())
    ...
    const ticketCreateResp = await request(app.getHttpServer())
      .post('/support/tickets')
      .set('x-extl-id', passengerExtl)
      .send({ subject: 'help', content: 'issue' })
      .expect(201);
    const ticketId = ticketCreateResp.body.id as string;
    ...
    await request(app.getHttpServer())
      .post('/messages')
      .set('x-extl-id', passengerExtl)
      .send({ tripId, recipientId: (await getUserId(app, driverExtl)), content: 'Hello!' })
      .expect(201);
    await messageNewP;
...
async function getUserId(app: INestApplication, extlId: string): Promise<string> {
  const res = await request(app.getHttpServer()).get(`/users/by-extl/${extlId}`).expect(200);
